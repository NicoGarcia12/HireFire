...
import { apify } from './apify.client.js';
...
function shouldFallbackToUrlActor(actor: JobsActorConfig, error: unknown): boolean {
  return actor.actorId === LEGACY_KEYWORD_ACTOR
...
/**
 * Busca ofertas en LinkedIn por Apify y aplica fallback al actor por URL si el actor legacy requiere alquiler.
 */
export async function searchJobs(params: JobSearchParams): Promise<Job[]> {
  ...
  try {
    ...
    if (!shouldFallbackToUrlActor(actor, error)) {
      throw error;
    }

    logger.warn(`Apify: actor legacy no disponible por alquiler vencido. Fallback automático a ${URL_SEARCH_ACTOR}.`);
